<template>
  <b-row>
    <b-col cols="12">
      <h2>
        Edit hero
        <router-link :to="{ name: 'ShowHero', params: { id: key } }">(Show hero)</router-link>
      ...
      <b-jumbotron>
        <b-form @submit="onSubmit">
          ...
          <b-form-group id="fieldsetHorizontal"
                    ...
                    breakpoint="md"
                    label="image">
              <b-form-input id="superpowers" v-model.trim="hero.image"></b-form-input>
          </b-form-group>
...
export default {
  ...
  methods: {
    onSubmit (evt) {
      evt.preventDefault()
      const updateRef = firebase.firestore().collection('heros').doc(this.$route.params.id);
      updateRef.set(this.hero).then(() => {
        this.key = ''
        this.hero.nickname = ''
        this.hero.real_name = ''
        ...
        this.hero.image2 = ''
         this.hero.image3 = ''
            this.hero.image4 = ''
             this.hero.image5 = ''
        router.push({ name: 'ShowHero', params: { id: this.$route.params.id }})
      })
      .catch((error) => {
